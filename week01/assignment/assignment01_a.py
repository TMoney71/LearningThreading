...
def main():
   ...
   #  setting the arg to the function
    t1 = threading.Thread(target=sumofall, args=(9,))
    t2 = threading.Thread(target=sumofall, args=(12,))
    t3 = threading.Thread(target=sumofall, args=(16,))

   #  starting the first thread
    t1.start()
    t1.join()
    assert global_sum == 45, f'The sum should equal 45 but instead was {global_sum}'

    global_sum = 0
    #  starting the second thread
    t2.start()
    t2.join()
    assert global_sum == 78, f'The sum should equal 45 but instead was {global_sum}'
    global_sum = 0
    #  starting the third thread
    t3.start()
    t3.join()
    assert global_sum == 136, f'The sum should equal 136 but instead was {global_sum}'



   #  sumofall(11)
   #  assert global_sum == 78, f'The sum should equal 78 but instead was {global_sum}' 
   #  sumofall(136)
   #  assert global_sum == 136, f'The sum should equal 136 but instead was {global_sum}'
